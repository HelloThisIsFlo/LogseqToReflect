...
def test_full_workspace_conversion(monkeypatch):
    ...
    try:
        ...
        for file in journal_files:
            assert re.match(r'\d{4}-\d{2}-\d{2}\.md', file), f"Journal file {file} not in correct format YYYY-MM-DD.md"
        ...
        if journal_files:
            ...
            with open(first_journal, "r") as f:
                ...
                assert re.search(r'# \w{3}, \w+ \d+\w{2}, \d{4}', content), "Date header not found in journal file"
        ...
        if page_files:
            ...
            with open(first_page, "r") as f:
                ...
                assert lines[0].startswith('# '), "Page doesn't start with a title header"
                ...
                if special_format_files:
                    special_page = os.path.join(output_dir, "pages", special_format_files[0])
                    with open(special_page, "r") as f:
                        content = f.read()
                        lines = content.strip().split('\n')
                        title = lines[0][2:]  # Remove "# " prefix
                        assert '/' in title, f"Title for file with ___ ({special_format_files[0]}) doesn't contain expected slash format"
                        
        print(f"\nConverted workspace created at: {output_dir}")
        print("You can manually inspect the results and then delete the directory when done.")
